...
public class SelectRouteToDownloadNotification implements DialogInterface.OnClickListener{
	...
	public void onClick(DialogInterface arg0, int which) {
		...
		MinRouteInfo routeToDownload = state.getAllRoutesAvailables().get(which);
		String[] urls;
		String[] names;
		...
		FileDownloadingThread t = new FileDownloadingThread(urls,names,ResourceManager.getInstance().getRootPath()+"/ar/");
		Toast.makeText(FRAGUEL.getInstance().getApplicationContext(), "Descargando ruta y archivos asociados. Espere por favor.", Toast.LENGTH_LONG).show();
		t.start();
	}
